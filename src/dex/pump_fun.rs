...
impl Pump {
    ...
    pub async fn buy(&self, mint: &str, amount: u64) -> Result<String> {
        ...
        let fee = Some(PriorityFee {
            limit: Some(1_000_000),  // Increased fee limit
            price: Some(1_000_000),  // Increased fee price
        });

        // Add retry logic for buy transaction
        let mut retries = 3;
        let mut last_error = None;

        while retries > 0 {
            match self.pump_client.buy(&mint_pubkey, amount, None, fee.clone()).await {
        ...
        Err(anyhow!("Buy failed after retries: {}", last_error.unwrap()))
    }
